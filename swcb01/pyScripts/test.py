...
def saveImgFeature2np():
    ...
    np.save(imgFeatureNpy_file_name,encoding_vectors)
...
def imageRetrieval(returnJson):
    ...
    imgFeatureNpy = np.load(imgFeatureNpy_file_name)

    #搜尋前10筆最相似的影像
    nn_indices=ann.get_nns_by_vector(pred,44)
    # nn_indices_2 = ann.get_nns_by_item(0, 100)
    ...
    if returnJson:
        ...
        for index in nn_indices:
            ...
            imgFeature = imgFeatureNpy[index]

            d = np.linalg.norm(pred - imgFeature)
            # d2 = distance.seuclidean(pred,imgFeature)
            # d2 = np.sqrt(2(1-0))

            imgDistanceList.append({"index":index,"distance":d})

            imgNamesList.append(i)

        newlist = sorted(imgDistanceList, key=lambda k: k['distance'])

        imgIdList = [name['name'].replace(".jpg","").split("EvenID_")[-1] for name in imgNamesList]

        imgDistanceList = np.array(imgDistanceList)
        pd.DataFrame(imgDistanceList).to_csv("xxx.csv")
        ...
        for i, item in enumerate(newlist,1):
            image = imgNpy[item['index']]
            ax = plt.subplot(10,5,i+5,)
            plt.xticks([])
            plt.yticks([])
            ax.axis("off")
            ax.text(0.5, -0.1, item['distance'], size=10, ha="center",
...
imageDir = os.path.join("..","CCTV")

imgNpy_file_name = os.path.join(abPath,'imgNpyFiles','swcb_imgnpy_1_5000.npy')
imgFeatureNpy_file_name = os.path.join(abPath,'imgNpyFiles','swcb_imgnpy_1_5000_feature.npy')
...
imageToSearch = os.path.join("testImage//type_0_EvenID_635779261877828341.jpg")
...
if ExecCode == 1:
    ...
    result, resultId = imageRetrieval(returnJson=False)
